...
def download(ftp, filename, dst_file_path):
    f = open(dst_file_path, "wb").write
    try:
        ftp.retrbinary("RETR %s" % filename, f, 1024 * 1024)
...
class DatasetInfo(Resource):
    @api.doc('copernicus自动下载地址')
    @ns.param('dir_name', '目录名称')
    @ns.param('filename', '文件名称')
    def get(self):
        dir_name = request.values.get('dir_name')
        filename = request.values.get('filename')
        dir_path = 'D:\\dataSource\\copernicus' + '\\' + dir_name
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        down_path = dir_path + '\\' + filename
        ftp = login()
        ftp.cwd('Core')
        if dir_name is not None:
            dir_n = dir_name.split("\\")
            for d in dir_n:
                ftp.cwd(d)
        download(ftp, filename, down_path)
        # return send_file(down_path, as_attachment=True)


@ns.route('/get_marine_elements')
class MarineElementsList(Resource):
    @api.doc('获取海洋要素目录或文件')
    # (如果多个，按照层级用', '拼接),获取初级目录无需传递此参数
    @ns.param('dir_name', '目录名称')
    def get(self):
        """获取海洋要素目录或文件"""
        dir_name = request.values.get('dir_name')
        ...
        if dir_name is not None:
            ...
            for d in dir_n:
                ftp.cwd(d)
        elements = ftp.nlst()
        return jsonify(errno=RET.OK, data=elements, errmsg="查询成功")
