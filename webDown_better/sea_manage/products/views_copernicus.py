...
class MetaInfo(Resource):
    ...
    @ns.param('cloud', '云覆盖率%')
    @ns.param('productLevel', '产品级别')
    @ns.param('order', '上升、下降')
    @ns.param('pageNum', '页数')
    @ns.param('polygon', '空间范围')
    def get(self):
        """获取列表"""
        ...
        polygon = request.values.get("polygon")
        cloud = request.values.get("cloud")
        productLevel = request.values.get("productLevel")
        polygon_str = ""
        if polygon is not None:
            polygon_str = "POLYGON(("
            for p in polygon.split(","):
                polygon_str = polygon_str + p + ","
            polygon_str = polygon_str + polygon.split(",")[0] + "," + polygon.split(",")[0] + "))"
        sum = (int(pageNum) - 1) * limit
        filter1 = ""
        filter2 = ""
        if polygon_str != "":
            footprint = '"Intersects(' + polygon_str + ')"'
            filter1 = "(    footprint:" + footprint + " )"
        if cloud is not None or platform is not None:
            filter2 = "(  (platformname:Sentinel-2"
            if cloud is not None:
                filter2 = filter2 + "  AND cloudcoverpercentage:" + cloud
            if platform is not None:
                filter2 = filter2 + "   AND filename:" + platform
            filter2 = filter2 + ")"
        if productLevel is not None:
            if filter2 == "":
                filter2 = "(  (platformname:Sentinel-3 AND productlevel:" + productLevel + "))"
            else:
                filter2 = filter2 + "   OR   (platformname:Sentinel-3 AND productlevel:" + productLevel + "))"
        else:
            if filter2 != "":
                filter2 = filter2 + ")"
        # ( % 20 % 20(footprint:{footprint} % 20AND % 20platformname:Sentinel-2 % 20AND % 20filename:{platform} % 20AND % 20producttype:S2MSI1C))
        filter = ""
        ...
        if filter1 == "" and filter2 != "":
            filter = filter2
        if filter == "":
            url = 'https://scihub.copernicus.eu/dhus/api/stub/products?offset={sum}&limit={limit}&sortedby={sortedby}&order={order}&filter=*'.format(
                sum=sum, limit=limit, sortedby=sortedby, order=order)
        else:
            url = 'https://scihub.copernicus.eu/dhus/api/stub/products?offset={sum}&limit={limit}&sortedby={sortedby}&order={order}&filter={filter}'.format(
                sum=sum, limit=limit, sortedby=sortedby, order=order,
                filter=filter)
        global cookie
        print(cookie)
        ...
        list = []
        down_path = 'D:/dataSource/shaobing2/'
        for i in response['products']:
            obj = {
                # "content": response.content,
                # "text": response.text,
                "thumbnail": "https://scihub.copernicus.eu/dhus/odata/v1/Products('" + i[
                    'uuid'] + "')/Products('Thumbnail')/$value",
            ...
            with open(down_path + '/' + i['uuid'] + '.json', 'w', encoding='utf-8') as f:
                f.write(str(i['wkt']))
            for a in i['summary']:
                obj[a.split(":")[0].strip()] = a.split(":")[1].strip()
                if a.split(":")[0].strip() == 'Filename':
                    obj[a.split(":")[0].strip()] = a.split(":")[1].strip() + ".zip"
            list.append(obj)
...
class Wkt(Resource):
    ...
    def get(self):
        """获取wkt"""
        filename = request.values.get('id')
        path = "D:\\dataSource\\shaobing2\\" + filename + ".json"
    ...
    def get(self):
        ...
        if len(name) != 0:
            ...
            submit = driver.find_element_by_xpath(
                "/html/body/div[@class='ng-scope']/div/div/div[@class='ng-isolate-scope'][7]/div/div[@id='userBadge']/div[2]/form[@class='ng-valid ng-dirty ng-valid-parse']/div[@class='row'][3]/button[@class='btn btn-default login-btn']")
